...
// with 4 blocks
// order 2 (answer button order reversed)
...
PennController("break_screen",
    newHtml("break_screen", "break_screen.html")
    ,
    newButton("Fortfahren")
        .wait()
)
...
PennController("postexp_survey2",
    newHtml("postexp_survey2", "postexp_survey2.html")
        .settings.radioWarning("Bitte geben Sie an, ob Sie die Studie ernsthaft bearbeitet haben.")
    ,
    newButton("Fortfahren")
    // Continue only if the html has been filled in:
        .wait(
          getHtml("postexp_survey2").test.complete()
              .failure(  getHtml("postexp_survey2").warn()  )
    )
)
.log("Survey", "ExperimentalSurvey")
...
Template("practice.csv", row =>
    ...
    newKey("showContext", "Enter") // start reading time: context sentence
        ...
        .log()
    ,
    newVar("ContextRT").global().set( v => Date.now() )  // start recording context RT
    ,
    getText("insSpace")
        .remove()
    ,
    ...
    newKey("removeContext", "Enter") // end reading time: context sentence
        ...
        .log()
    ,
    getVar("ContextRT").set( v => Date.now() - v )  // end recording context RT
    ,
  ...
  .log("ExpBlock", row.Block)
  .log("ExpCritical", row.Critical)
  .log("ExpTarget", row.Target)
  .log("ExpPlausible", row.Plausible)
  .log("ExpContextRT" , getVar("ContextRT"))
  .log("ExpDecisionRT" , getVar("DecisionRT"))
)

// Experimental trials
Template("list1_order2.csv", row =>
  // newTrial("experiment",
  newTrial(row.Block,  // proceed by block
