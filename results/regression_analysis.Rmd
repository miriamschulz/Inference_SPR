...
```{r}
# Create binary predictor for association levels
df$Associated <- as.factor(ifelse(df$Cond %in% c("A", "C"), 1, 0))

# Set contrasts
#contrasts(df$Cond) <- car::contr.Sum(levels(df$Cond))
#contrasts(df$Cond)
contrasts(df$Plausible) <- contr.sum(2)
contrasts(df$Associated) <- contr.sum(2)

contrasts(df$Plausible)
contrasts(df$Associated)
...
f <- as.formula(logRT ~ Associated*Plausible + logRT_precrit1 + (1|Subject) + (1|Item))
...
**Observations:**
TODO: adjust for condition-coded model
