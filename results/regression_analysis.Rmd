...
```{r, results = FALSE, echo = FALSE, fig.width=9, fig.height=5}
rand.ef.complex <- " + (1+Inference+Coherence+Association || Subject) + (1+Inference+Coherence+Association || Item)"
m.formula <- makeFormula(DV,
                         IVs = "1 + logRT_precrit + Coherence +
                         Association*Inference",
                         rand.ef = rand.ef.complex)
summary(lmer(m.formula, data=crit))
summary(lmer(m.formula, data=spill1))
summary(lmer(m.formula, data=spill2))
```


```{r, include = FALSE, echo = FALSE, fig.width=9, fig.height=5}
# In addition, we can also add ContextRT, DecisionRT and TrialNumber to test
