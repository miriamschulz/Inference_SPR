...
This script contains an regression-based reading time analysis of the inference reading time data (SPR) based on the rERP modeling approach in Brouwer, Delogu & Crocker (2020). 
...
for (r in-1:2) {
  ...
  hist(
      ...
      main = "text"
      #main = cat(sprintf("RTs in region: %s (50 <= RT <= 1000ms)", r))
  )
}
...
## Models with a single predictor (+ the intercept)
...
## Models with more than 1 additional predictor
...
p1 <- plotSPR(m, "InterceptCoherencePrecrit1Interaction", "Estimates: Full model w/o Association",
        y.unit, y.range)
...
p2 <- plotSPR(m, "InterceptAssocPrecrit1Interaction", "Estimates: Full model w/o Coherence",
        y.unit, y.range)
...
p3 <- plotSPR(m, "InterceptAssocCoherenceInteraction", "Estimates: Full model w/o Precrit1",
        y.unit, y.range)
...
p4 <- plotSPR(m, "InterceptAssocCoherencePrecrit1", "Estimates: Full model w/o interaction",
        y.unit, y.range)
...
# Add everything except the interaction term 
m$InterceptOnly <- m$`Coef_(Intercept)`
p.intercept <- plotSPR(m, "InterceptOnly", "Estimates: Intercept only",
        y.unit, y.range)  

# Observed data 
p.observed <- plotSPR(m, DV, "Observed RTs", y.unit, y.range)

p <- grid.arrange(p.intercept,  p2, p4, p1, p3, 
                p5,
             nrow=2,
             top = textGrob("Setting individual predictors to zero", gp=gpar(fontsize=14, fontface=2)))
filename <- paste0("./plots/", DV, "_", "Zero_plots", ".png")
