...
survey <- read.csv("results_demographics.csv")
```
...
xtabs(~ Item + Cond, df.mini)
# Check which order each participant saw:
xtabs(~ Subject + Order, df.mini)
# Check that max item/cond entry is equal to N of participants/list = 10:
max(xtabs(~ Item + Cond, df.mini))
# Check that min item/cond entry is equal to N of participants/list = 10:
min(xtabs(~ Item + Cond, df.mini))

xtabs(~ Subject + Cond, df.mini)       # confirm that each subj saw X items per condition
...
reads <- droplevels(filter(df, Item < 100))
range(reads$RT)
```
...
(means.cond.all <-
    merge(means.cond.all, means.cond.allSE, by = c("Cond")))
...
#TODO: remove all datapoints from trial where single datapoint removed
...
reads <- filter(reads, Region %in% c(-2:3, 99))
...
reads <- filter(reads, Region %in% -1:2)
```
