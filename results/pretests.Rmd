...
barplotRatings <- function(df,
  ...
  if (plotname == "prodnorms") {
    p <- p + ylab("Production Percentage")
  }
...
densityplotRatings <- function(df,
  ...
  p <- ggplot(data = df,
    ...
    xlim(xrange) +
    #scale_x_continuous(breaks = xrange[1]:xrange[2]) +
...
(means.prod <- aggregMeans(df.ab, as.formula(ProductionPercentage ~ Cond), n=40))
(means.assoc <- aggregMeans(df, as.formula(AssociationRating ~ Cond), n=40))
