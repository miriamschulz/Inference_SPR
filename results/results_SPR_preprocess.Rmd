...
```{r}
items <- read.csv("items_selected_prod+assoc+inference+coherence.csv", header = TRUE)
...
```{r}
xtabs(~ Item + Cond, droplevels(ratings[ratings$Item < 100, ]))       # confirm that each item appeared equal times in all conds
xtabs(~ Subject + Order, droplevels(ratings[ratings$Item < 100, ]))   # which order each participant saw
max(xtabs(~ Item + Cond, droplevels(ratings[ratings$Item < 100, ])))  # has to be n of participants/list (10)
min(xtabs(~ Item + Cond, droplevels(ratings[ratings$Item < 100, ])))  # has to be n of participants/list (10)
```
...
(code chunk skipped, see Rmd script)
